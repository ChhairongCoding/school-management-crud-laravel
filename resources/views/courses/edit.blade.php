    ...
    <div class="bg-white p-6 shadow rounded space-y-4">
        ...
        <form action="{{ route('courses.update', $course->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div class="mb-4">
                ...
                @if ($course->image_url)
                    <img src="{{ asset('storage/' . $course->image_url) }}" alt="{{ $course->title }}" class="w-32 mb-2">
                @endif
            ...
            <div class="flex justify-end">
                <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 mr-5">Update</button>
                <button type="button" class="px-4 py-2 rounded border border-gray-300 text-gray-700 hover:border-blue-700 hover:text-blue-700" onclick="window.location.href='{{ route('courses.index') }}'">Cancel</button>
            </div>
        </form>
    </div>
@endsection
